...
export function remap(minIn, maxIn, minOut, maxOut, v) {
...
export function clampedRemap(minIn, maxIn, minOut, maxOut, v) {
...
// Beautiful graph:       /          \
//                  _____.            ._____
export function smoothpulse(inStart, inEnd, outStart, outEnd, v) {
    return easeInOutCubic(clampedRemap(inStart, inEnd, 0, 1, v) *
        (1 - clampedRemap(outStart, outEnd, 0, 1, v)));
}
export function removeAnimations(al, names) {
    return al.filter(function (a) { return !names.includes(a.name); });
}
